...
def create_app(config_class=Config):
    ...
    # CORS (usa lista desde Config; en tu dataclass usa default_factory para listas)
    origins = getattr(config_class, "cors_origins", ["*"])
    ...
    def handle_ex(e):
        ...
        return jsonify({"error":"Internal Server Error","message":"Ha ocurrido un error interno","status_code":500}), 500

    @app.before_request
    def log_req(): logger.info("%s %s - %s", request.method, request.path, request.remote_addr)
    @app.after_request
    def log_resp(r): logger.info("Response: %s", r.status_code); return r

    @app.get("/api/health")
    def health():
        return jsonify({"status":"healthy","timestamp":datetime.now().isoformat(),"version":"1.0.0","database":db_manager.health_check()})

    @app.get("/api/info")
    def info():
        return jsonify({"name":"Sistema de Predicción de Quiebras","version":"1.0.0","endpoints":{
            "auth":"/api/auth","empresas":"/api/empresas","predicciones":"/api/predicciones","alertas":"/api/alertas","etl":"/api/etl","dashboard":"/api/dashboard"}})
